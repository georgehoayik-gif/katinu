...
# ==============================
    ...
    if usar_filtro_api and data_inicio_api is not None:
        st.info(f"🔎 Buscando via API: {data_inicio_api} até {data_fim_api}")
        df = buscar_dados(data_inicio_api, data_fim_api)
    else:
        st.info("🔎 Buscando últimos 30 dias (modo padrão)")
    df = buscar_dados()
...
#========================================================
# 📥 EXPORTAÇÃO EXCEL (VERSÃO FINAL CORRIGIDA)
#========================================================
...
def gerar_excel(df, mesas_selecionadas):
    output = BytesIO()
    ...
    chart_rank = BarChart()
    data_rank = Reference(ws_rank, min_col=2, min_row=1, max_row=ws_rank.max_row)
    cats_rank = Reference(ws_rank, min_col=1, min_row=2, max_row=ws_rank.max_row)

    chart_rank.add_data(data_rank, titles_from_data=True)
    chart_rank.set_categories(cats_rank)
    chart_rank.title = "Ranking de Mesas"

    ws_rank.add_chart(chart_rank, "E2")
...
# BOTÃO DOWNLOAD
# ==============================
excel_file = gerar_excel(df, mesas_selecionadas)

st.download_button(
    label="📥 Baixar Excel",
    data=excel_file,
    file_name="relatorio_tickets.xlsx",
